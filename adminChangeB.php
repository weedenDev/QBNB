		...
		<?php
			if(isset($_SESSION['id'])){
			   // include database connection
				...
				// SELECT query
					$currentID = $_SESSION['id'];
			        $query = "SELECT consumer, owner_ID, house_ID, booking_period, status, address, city, postal_code, features,type, price, available FROM (Bookings NATURAL JOIN Rental_Properties)";
			        // prepare query for execution
			        $stmt = $con->prepare($query);
			        // bind the parameters. This is the best way to prevent SQL injection hacks.
			        //$stmt->bind_Param("s", $_SESSION['id']);
				...
				<div class="col-lg-12">
					...
					<?php
						...
						while($myrow = $result->fetch_assoc()) {
							...
							echo "<form name='cbook' id='cbook' action='admincancelbooking.php' method='post'><tr>"."<td>".$myrow['owner_ID']."</td>"."<td>".$myrow['house_ID']."</td>"."<td>".$myrow['address']."</td>"."<td>".$myrow['city']."</td>"."<td>".$myrow['postal_code']."</td>"."<td>".$myrow['type']."</td>"."<td>".$myrow['features']."</td>"."<td>".$myrow['price']."</td>"."<td>".$myrow['booking_period']."</td>"."<td>".$myrow['available']."</td>"."<td>".$myrow['status']."</td>"."<input class='dontshow' type='text' id='conB' name='conB' value='".$myrow['consumer']."'/><input class='dontshow' type='text' id='houseBB' name='houseBB' value='".$myrow['house_ID']."'/>"."<td><input type='submit' name='cAbookBtn' id='cAbookBtn' value='Cancel'/></td>"."</form>"."</tr>"."<br>";
						}
					?>
					</table>
				</div>
