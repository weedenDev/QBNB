		...
		<?php
			if(isset($_SESSION['id'])){
			   // include database connection
				...
				$currentID = $_SESSION['id'];
				// SELECT query
			        $query = "SELECT house_ID, suppliers, address, city, postal_code, type, features, price FROM Rental_Properties WHERE suppliers='$currentID'";
			        // prepare query for execution
			        $stmt = $con->prepare($query);
			        // bind the parameters. This is the best way to prevent SQL injection hacks.
			        //$stmt->bind_Param("s", $_SESSION['id']);
		...
		<div class="container">
			<div id='updateProfile' class="row">
				<div class="col-md-12">
					...
					<?php
						echo "<tr class='heading'>"."<td>Supplier</td>"."<td>Address</td>"."<td>City</td>"."<td>Postal Code</td>"."<td>Room</td>"."<td>Features</td>"."<td>Price</td>"."<td>Delete</td>"."</tr>";
						while($myrow = $result->fetch_assoc()) {
							echo "<form name='deleteHouse' id='deleteHouse' action='deleteHouse.php' method='post'><tr>"."<td>".$myrow['suppliers']."</td>"."<td>".$myrow['address']."</td>"."<td>".$myrow['city']."</td>"."<td>".$myrow['postal_code']."</td>"."<td>".$myrow['type']."</td>"."<td>".$myrow['features']."</td>"."<td>".$myrow['price']."</td>"."<td><input type='submit' id='deleteBtn' name='deleteBtn' value='Delete'/></td><input class='dontshow' type='text' id='addressB' name='addressB' value='".$myrow['address']."'/><input class='dontshow' type='text' id='cityB' name='cityB' value='".$myrow['city']."'/><input class='dontshow' type='text' id='postal' name='postal' value='".$myrow['postal_code']."'/><input class='dontshow' type='text' id='typeB' name='typeB' value='".$myrow['type']."'/><input class='dontshow' type='text' id='featuresB' name='featuresB' value='".$myrow['features']."'/><input class='dontshow' type='text' id='priceB' name='priceB' value='".$myrow['price']."'/><input class='dontshow' type='text' id='houseB' name='houseB' value='".$myrow['house_ID']."'/><input class='dontshow' type='text' id='supplierB' name='supplierB' value='".$myrow['suppliers']."'/></form>"."</tr>"."<br>";
						}
					?>
					</table>
					<div class="col-md-4">
